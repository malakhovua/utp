...
Процедура УстановитьПараметрыКолонокСуммы( ФормаТабЧасть, УпрУчет, БухУчет) Экспорт
	...
	Если УпрУчет Тогда
		ФормаТабЧасть.Колонки.Цена.ТекстШапки = НСтр("ru = 'Цена'; uk = 'Ціна'");
...
"Открыть обработку, устанавливающую ведение партионного учета?';uk='Даний спосіб оцінки може бути встановлений, якщо по рахунках МВЗ встановлене ведення партіонного обліку. "
...
Процедура ОбновитьКолонкиСуммТабПоле(ТабПоле, СписокЦенОстатков, ВВалютеЦены, ВалютаЦены, ОбновлятьШапкиКолонок = Истина, КоличествоБазовыхКолонок) Экспорт
	...
	ПостфиксЦены = ?(ВыводитьВУстановленнойВалюте, " " + НСтр("ru = 'Цена'; uk = 'Ціна'") + ", " + СокрЛП(ВалютаЦены), " " + НСтр("ru = 'Цена'; uk = 'Ціна'"));
...
Процедура ОбновитьШапкиКолонокОстатков(ТабПоле, ЕдиницаХранения) Экспорт
	...
	ТабПоле.Колонки.Общий.ТекстШапки              = НСтр("ru = 'Общий'; uk = 'Загальний'") + СтрокаПредставленияЕдиницы;
	ТабПоле.Колонки.Свободный.ТекстШапки          = НСтр("ru = 'Свободный'; uk = 'Вільний'") + СтрокаПредставленияЕдиницы;
	ТабПоле.Колонки.СвободныйОжидаемый.ТекстШапки = НСтр("ru = 'Своб.ожид.'; uk = 'Віл.очік.'") + СтрокаПредставленияЕдиницы;
