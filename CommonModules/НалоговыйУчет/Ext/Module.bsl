...
Процедура РазбитьПроводкуПоНДСНаПервоеВтороеСобытие(ТаблицаПоВторомуСобытиюНал, ДвиженияДокумента, Проводка,
	...
	Если СуммаВторогоСобытия > 0 Тогда
		...
		Если РасчетыВВалюте Тогда
			СуммаПроводкиВтороеСобытие = Ценообразование.РассчитатьСуммуНДСсУчетомПогрешности(СуммаВторогоСобытия * КурсЗачетаАванса, Истина, Ложь, СтавкаНДС) * ?(Проводка.Сумма<0, -1, 1);
		КонецЕсли;
		...
		Если СуммаПроводки > СуммаВторогоСобытия Тогда
			...
			ПроводкаВтороеСобытие.Содержание = ПроводкаВтороеСобытие.Содержание + НСтр("ru=', второе событие';uk=', друге подія'");	
		...
		Иначе
			...
			Проводка.Содержание = Проводка.Содержание + НСтр("ru=', второе событие';uk=', друге подія'");	
			...
			Если СуммаВторогоСобытия > 0 Тогда
				...
				Если СуммаПроводки > СуммаВторогоСобытия Тогда	
					...
					Если РасчетыВВалюте Тогда
						...
						СуммаВторогоСобытияРегл = Ценообразование.РассчитатьСуммуНДСсУчетомПогрешности(СуммаВторогоСобытия * СтруктураКурсаДокумента.Курс / СтруктураКурсаДокумента.Кратность, Истина, Ложь, СтавкаНДС);						
				...
				Иначе
					...
					Если РасчетыВВалюте Тогда
						...
						СуммаПроводкиВтороеСобытие	= Ценообразование.РассчитатьСуммуНДСсУчетомПогрешности(СуммаПроводки * КурсЗачетаАванса, Истина, Ложь, СтавкаНДС) * ?(Проводка.Сумма<0, -1, 1);
...
Функция ПолучитьСуммуПроводкиПоНДС(ТаблицаПоВторомуСобытиюНал, СуммаНДС, 
	...
	Если РасчетБазыНДС Тогда
		...
		Возврат Ценообразование.РассчитатьСуммуНДСсУчетомПогрешности(СуммаБезНДСРегл,
											Истина,
											Ложь,
											СтавкаНДС);
...
Функция ДвиженияПоРегистрамНалоговогоУчетаПлатежныхДокументов(ДокументОбъект, СтруктураШапкиДокумента, РеестрПлатежей, Отказ=Ложь) Экспорт
	...
	СтруктураШапкиДокумента.Свойство("ВидОперации", ВидОперации);
	ОпределениеНаправленияДвижения = БухгалтерскийУчетРасчетовСКонтрагентами.ОпределениеНаправленияДвиженияДляДокументаДвиженияДенежныхСредств(СтруктураШапкиДокумента.ВидДокумента,ВидОперации);
	НаправлениеДвижения = ОпределениеНаправленияДвижения.Направление;
	РасчетыВозврат = ОпределениеНаправленияДвижения.РасчетыВозврат;
	...
	Для каждого СтрокаПлатежа из РеестрПлатежей Цикл
		...
		Если СтруктураШапкиДокумента.ЕстьНДС Тогда
			...
			Если  СуммаПроводки <> 0 
				...
				Проводка.Содержание                             = ?(ЭтоПродажа,НСтр("ru='НДС: налоговые обязательства';uk=""ПДВ: податкові зобов'язання""",Локализация.КодЯзыкаИнформационнойБазы()),
																	 НСтр("ru='НДС: налоговый кредит';uk='ПДВ: податковий кредит'",Локализация.КодЯзыкаИнформационнойБазы())) 
																	 + ?(ЭтоВозврат, ", (сторно) оплата", ", оплата");
				Проводка.НомерЖурнала                           = "";
