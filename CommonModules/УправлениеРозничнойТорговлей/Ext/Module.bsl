...
Процедура ПечатьЭтикеток(Товары = Неопределено) Экспорт
	...
	Исключение
		Сообщить(НСтр("ru='Компонента Печать штрихкодов не установлена на данном компьютере!';uk=""Компонента Друк штрихкодів не встановлена на даному комп'ютері!"""), СтатусСообщения.Важное);
		Возврат;
	КонецПопытки;
...
Процедура ПроверитьКорректностьИнформационнойКарты(ИнформационнаяКарта, ТипКарты) Экспорт
	...
	Если ЗначениеЗаполнено(ИнформационнаяКарта) Тогда
		Если ИнформационнаяКарта.ТипКарты <> ТипКарты Тогда
			Сообщить(НСтр("ru='В данном контексте можно выбирать только дисконтные карты.';uk='В даному контексті можна вибирати тільки дисконтні картки.'"));
