...
Функция ОпределитьТипШтрихкода(Знач Штрихкод) Экспорт
	...
	Если ТипШтрихкодаЗнач = "CODE128"  Тогда // EAN128
		...
		Если КодСимвола(Штрихкод, 1) = 40 Тогда //Или СтрНайти(Штрихкод, КодСимвола(29)) > 0  Тогда // umk не используется в режиме совместимости
...
Функция СформироватьТекстНефискальногоДокумента(ТипДокумента, ОбщиеПараметры, ШиринаСтроки = 32, ДополнительныйТекст = Неопределено, ФорматФФД = "1.1") Экспорт
	...
	Если ТипДокумента = 0 Тогда
		...
		ТипыЧеков.Вставить(1, НСтр("ru='ПРИХОД';uk='ПРИХІД'"));          // Чек продажи     
		ТипыЧеков.Вставить(2, НСтр("ru='ВОЗВРАТ ПРИХОДА';uk='ПОВЕРНЕННЯ ПРИХІДУ'")); // Чек возврата продажи 
	...
	ИначеЕсли ТипДокумента = 3 Тогда
		ТипЧека = НСтр("ru='ВЫЕМКА';uk='ВИЇМКА'");  
	...
	Если ПечатьКлише Тогда 
		...
		Если ТипДокумента = 0  Тогда
			...
			Если НЕ ПустаяСтрока(ОбщиеПараметры.Получатель) Тогда
				Текст = Текст + ПостроитьПоле(" " + НСтр("ru='ПОКУПАТЕЛЬ:';uk='ПОКУПЕЦЬ:'") + ОбщиеПараметры.Получатель, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ПолучательИНН) Тогда
				Текст = Текст + ПостроитьПоле(" " + НСтр("ru='ИНН ПОКУПАТЕЛЯ:';uk='ІПН ПОКУПЦЯ:'") + ОбщиеПараметры.ПолучательИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			...
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПОСТАВЩИК:';uk='ПОСТАЧАЛЬНИК:'"), ОбщиеПараметры.ДанныеПоставщика.Наименование, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПОСТАВЩИК ТЛФ:';uk='ПОСТАЧАЛЬНИК ТЛФ:'"), ОбщиеПараметры.ДанныеПоставщика.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПОСТАВЩИК ИНН:';uk='ПОСТАЧАЛЬНИК ІПН:'"), ОбщиеПараметры.ДанныеПоставщика.ИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Операция) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПЛ.АГЕНТ ОПЕР.:';uk='ПЛ.АГЕНТ ОПЕР.:'"), ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Операция, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Телефон) Тогда
				Текст = Текст + ВыстроитьПоля("  " + НСтр("ru='ПЛ.АГЕНТ.ТЛФ.:';uk='ПЛ.АГЕНТ.ТЛФ.:'"), ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ОПЕРАТОР ПЕРЕВОДА:';uk='ОПЕРАТОР ПЕРЕКАЗУ:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Наименование, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Телефон) Тогда
				Текст = Текст + ВыстроитьПоля("   " + НСтр("ru='ТЛФ.ОП.ПЕРЕВОДА:';uk='ТЛФ.ОП.ПЕРЕВОДА:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Адрес) Тогда
				Текст = Текст + ВыстроитьПоля("   " + НСтр("ru='АДР.ОП.ПЕРЕВОДА:';uk='АДР.ОП.ПЕРЕВОДА:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Адрес, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.ИНН) Тогда
				Текст = Текст + ВыстроитьПоля("   " + НСтр("ru='ИНН ОП.ПЕРЕВОДА:';uk='ІПН ОП.ПЕРЕКАЗУ:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.ИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:';uk='ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:'"), ОбщиеПараметры.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если ФорматФФД = "1.1" Тогда
					...
					Если НЕ ПустаяСтрока(КТ) Тогда
						Текст = Текст + ПостроитьПоле(" " + НСтр("ru='КТ:';uk='КТ:'") + КТ, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ПозицияЧека.КодСтраныПроисхожденияТовара) Тогда
						Текст = Текст + ПостроитьПоле(" " + НСтр("ru='КОД СТРАНЫ:';uk='КОД КРАЇНИ:'") + ПозицияЧека.КодСтраныПроисхожденияТовара, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
					Если НЕ ПустаяСтрока(ПозицияЧека.НомерТаможеннойДекларации) Тогда
						Текст = Текст + ПостроитьПоле(" " + НСтр("ru='ДЕКЛАРАЦИЯ:';uk='ДЕКЛАРАЦІЯ:'") + ПозицияЧека.НомерТаможеннойДекларации, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
				...
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПОСТАВЩИК:';uk='ПОСТАЧАЛЬНИК:'"), ПозицияЧека.ДанныеПоставщика.Наименование, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПОСТАВЩИК ТЛФ.:';uk='ПОСТАЧАЛЬНИК ТЛФ.:'"), ПозицияЧека.ДанныеПоставщика.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля("  " + НСтр("ru='ПОСТАВЩИК ИНН:';uk='ПОСТАЧАЛЬНИК ІПН:'"), ПозицияЧека.ДанныеПоставщика.ИНН, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Операция) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПЛ.АГЕНТ ОПЕР.:';uk='ПЛ.АГЕНТ ОПЕР.:'"), ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Операция, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Телефон) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru=' ПЛ.АГЕНТ ТЛФ.:';uk=' ПЛ.АГЕНТ ТЛФ.:'"), ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ОПЕРАТОР ПЕРЕВОДА:';uk='ОПЕРАТОР ПЕРЕКАЗУ:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Наименование, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.Телефон) Тогда
					Текст = Текст + ВыстроитьПоля("   " + НСтр("ru='ТЛФ.ОП.ПЕРЕВОДА:';uk='ТЛФ.ОП.ПЕРЕВОДА:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.Адрес) Тогда
					Текст = Текст + ВыстроитьПоля("   " + НСтр("ru='АДР.ОП.ПЕРЕВОДА:';uk='АДР.ОП.ПЕРЕВОДА:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Адрес, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.ИНН) Тогда
					Текст = Текст + ВыстроитьПоля("   " + НСтр("ru='ИНН ОП.ПЕРЕВОДА:';uk='ІПН ОП.ПЕРЕКАЗУ:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.ИНН, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон) Тогда
					Текст = Текст + ВыстроитьПоля("  " + НСтр("ru='ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:';uk='ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:'"), ПозицияЧека.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Если ФорматФФД = "1.0" Тогда // ФФД 1.0
			Если СуммаНаличными > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='НАЛИЧНЫМИ';uk='ГОТІВКОЮ'"), "=" + Формат(СуммаНаличными, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаЭлектронными > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ЭЛЕКТРОННЫМИ';uk='ЕЛЕКТРОННИМИ'"), "=" + Формат(СуммаЭлектронными, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПостоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ЭЛЕКТРОННЫМИ';uk='ЕЛЕКТРОННИМИ'"), "=" + Формат(СуммаПостоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПредоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ЭЛЕКТРОННЫМИ';uk='ЕЛЕКТРОННИМИ'"), "=" + Формат(СуммаПредоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
			Если СуммаПредоставлением > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ЭЛЕКТРОННЫМИ';uk='ЕЛЕКТРОННИМИ'"), "=" + Формат(СуммаПредоставлением, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
		Иначе // ФФД 1.0.5 & ФФД 1.1
			...
			КонецЕсли;
			Если СуммаЭлектронными > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='БЕЗНАЛИЧНЫМИ';uk='БЕЗГОТІВКОВИМИ'"), "=" + Формат(СуммаЭлектронными, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПредоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПРЕДВАРИТЕЛЬНАЯ';uk='ПОПЕРЕДНЯ'"), "=" + Формат(СуммаПредоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
			Если СуммаПостоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ПОСЛЕДУЮЩАЯ';uk='НАСТУПНА'"), "=" + Формат(СуммаПостоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПредоставлением > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='ИНАЯ ФОРМА';uk='ІНША ФОРМА'"), "=" + Формат(СуммаПредоставлением, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
		КонецЕсли; 
		...
		Если СуммаОплаты - СуммаЧека > 0 Тогда
			...
			Текст = Текст + ВыстроитьПоля(" " + НСтр("ru='СДАЧА';uk='РЕШТА'"), "=" + Формат(СуммаОплаты - СуммаЧека, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли; 
		...
		Если ОбщиеПараметры.Свойство("СистемаНалогообложения") Тогда
			...
			Текст = Текст + ВыстроитьПоля(НСтр("ru='СНО:';uk='СО:'"), СистемаНалогообложения, ШиринаСтроки) + Символы.ПС;
		КонецЕсли;          
		...
		Если СуммаПозицийНДС7 > 0 Тогда
			...
			Текст = Текст + ВыстроитьПоля(НСтр("ru=' СУММА НДС 7%';uk=' СУМА ПДВ 7%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС120 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС120 > 0, СуммаНДС120, СуммаПозицийНДС120 / 120 * 20);
			Текст = Текст + ВыстроитьПоля(НСтр("ru=' СУММА НДС 20/120%';uk=' СУМА ПДВ 20/120%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС107 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС107 > 0, СуммаНДС107, СуммаПозицийНДС107 / 107 * 7);
			Текст = Текст + ВыстроитьПоля(НСтр("ru=' СУММА НДС 7/107%';uk=' СУМА ПДВ 7/107%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
...
Функция НаименованиеСистемыНалогообложения(СистемаНалогообложения, Кратко = Ложь) Экспорт;
	...
	СистемыНалогообложения = Новый Соответствие();
	Если Кратко Тогда
		СистемыНалогообложения.Вставить(0, НСтр("ru='ОСН';uk='ОСН'"));
		СистемыНалогообложения.Вставить(1, НСтр("ru='УСН доход';uk='ССО дохід'"));
		СистемыНалогообложения.Вставить(2, НСтр("ru='УСН доход-расход';uk='УСН дохід-витрата'"));
		СистемыНалогообложения.Вставить(3, НСтр("ru='ЕНВД';uk='ЄП'"));
		СистемыНалогообложения.Вставить(4, НСтр("ru='ЕСН';uk='ЕСН'"));
		СистемыНалогообложения.Вставить(5, НСтр("ru='Патент';uk='Патент'"));
	Иначе
		СистемыНалогообложения.Вставить(0, НСтр("ru='Общая';uk='Загальна'"));
		СистемыНалогообложения.Вставить(1, НСтр("ru='Упрощенная Доход';uk='Спрощена Доход'"));
		СистемыНалогообложения.Вставить(2, НСтр("ru='Упрощенная Доход минус Расход';uk='Спрощена Доход мінус Витрата'"));
		СистемыНалогообложения.Вставить(3, НСтр("ru='Единый налог на вмененный доход';uk='Єдиний податок'"));
		СистемыНалогообложения.Вставить(4, НСтр("ru='Единый сельскохозяйственный налог';uk='Єдиний сільськогосподарський податок'"));
		СистемыНалогообложения.Вставить(5, НСтр("ru='Патентная система налогообложения';uk='Патентна система оподаткування'"));
	КонецЕсли;
...
Функция НаименованиеПризнакаПредметаРасчета(ПризнакПредметаРасчета) Экспорт;
	...
	ПризнакиПредметаРасчета.Вставить(1 , НСтр("ru='ТОВАР';uk='ТОВАР'"));
	ПризнакиПредметаРасчета.Вставить(2 , НСтр("ru='ПОДАКЦИЗНЫЙ ТОВАР';uk='ПІДАКЦИЗНИЙ ТОВАР'"));
	ПризнакиПредметаРасчета.Вставить(3 , НСтр("ru='РАБОТА';uk='РОБОТА'"));
	ПризнакиПредметаРасчета.Вставить(4 , НСтр("ru='УСЛУГА';uk='ПОСЛУГА'"));
	ПризнакиПредметаРасчета.Вставить(5 , НСтр("ru='СТАВКА АЗАРТНОЙ ИГРЫ';uk='СТАВКА АЗАРТНОЇ ГРИ'"));
	ПризнакиПредметаРасчета.Вставить(6 , НСтр("ru='ВЫИГРЫШ АЗАРТНОЙ ИГРЫ';uk='ВИГРАШ АЗАРТНОЇ ГРИ'"));
	ПризнакиПредметаРасчета.Вставить(7 , НСтр("ru='ЛОТЕРЕЙНЫЙ БИЛЕТ';uk='ЛОТЕРЕЙНИЙ БІЛЕТ'"));
	ПризнакиПредметаРасчета.Вставить(8 , НСтр("ru='ВЫИГРЫШ ЛОТЕРЕИ';uk='ВИГРАШ ЛОТЕРЕЇ'"));
	ПризнакиПредметаРасчета.Вставить(9 , НСтр("ru='ПРЕДОСТАВЛЕНИЕ РИД';uk='НАДАННЯ РІД'"));
	ПризнакиПредметаРасчета.Вставить(10, НСтр("ru='ПЛАТЕЖ';uk='ПЛАТІЖ'"));
	ПризнакиПредметаРасчета.Вставить(11, НСтр("ru='АГЕНТСКОЕ ВОЗНАГРАЖДЕНИЕ';uk='АГЕНТСЬКА ВИНАГОРОДА'"));
	ПризнакиПредметаРасчета.Вставить(12, НСтр("ru='СОСТАВНОЙ ПРЕДМЕТ РАСЧЕТА';uk='СКЛАДЕНИЙ ПРЕДМЕТ РОЗРАХУНКУ'"));
	ПризнакиПредметаРасчета.Вставить(13, НСтр("ru='ИНОЙ ПРЕДМЕТ РАСЧЕТА';uk='ІНШИЙ ПРЕДМЕТ РОЗРАХУНКУ'"));
	ПризнакиПредметаРасчета.Вставить(14, НСтр("ru='ИМУЩЕСТВЕННОЕ ПРАВО';uk='МАЙНОВЕ ПРАВО'"));
	ПризнакиПредметаРасчета.Вставить(15, НСтр("ru='ВНЕРЕАЛИЗАЦИОННЫЙ ДОХОД';uk='ПОЗАРЕАЛІЗАЦІЙНІ ДОХОДИ'"));
	ПризнакиПредметаРасчета.Вставить(16, НСтр("ru='СТРАХОВЫЕ ВЗНОСЫ';uk='СТРАХОВІ ВНЕСКИ'"));
	ПризнакиПредметаРасчета.Вставить(17, НСтр("ru='ТОРГОВЫЙ СБОР';uk='ТОРГОВИЙ ЗБІР'"));
	ПризнакиПредметаРасчета.Вставить(18, НСтр("ru='КУРОРТНЫЙ СБОР';uk='КУРОРТНИЙ ЗБІР'"));
	ПризнакиПредметаРасчета.Вставить(19, НСтр("ru='ЗАЛОГ';uk='ЗАСТАВА'"));
...
Функция НаименованиеПризнакПредметаРасчетаПоКоду(КодПризнакаПредметаРасчета) Экспорт;
	...
	ПризнакиПредметаРасчета.Вставить(1, НСтр("ru='Доход от долевого участия в других организациях';uk='Дохід від пайової участі в інших організаціях'"));
	ПризнакиПредметаРасчета.Вставить(2, НСтр("ru='Доход в виде курсовой разницы, образующейся вследствие отклонения курса продажи (покупки) иностранной валюты от официального курса';uk='Дохід у вигляді курсової різниці, що утворюється внаслідок відхилення курсу продажу (покупки) іноземної валюти від офіційного курсу'"));
	ПризнакиПредметаРасчета.Вставить(3, НСтр("ru='Доход в виде подлежащих уплате должником штрафов, пеней и (или) иных санкций за нарушение договорных обязательств';uk='Дохід у вигляді штрафів, пеней та (або) інших санкцій, що підлягають сплаті боржником, за порушення договірних зобов`язань'"));
	ПризнакиПредметаРасчета.Вставить(4, НСтр("ru='Доход от сдачи имущества (включая земельные участки) в аренду (субаренду)';uk='Дохід від здачі майна (включаючи земельні ділянки) в оренду (суборенду)'"));
	ПризнакиПредметаРасчета.Вставить(5, НСтр("ru='Доход от предоставления в пользование прав на результаты интеллектуальной деятельности';uk='Дохід від надання в користування прав на результати інтелектуальної діяльності'"));
	ПризнакиПредметаРасчета.Вставить(6, НСтр("ru='Доход в виде процентов, полученных по договорам займа и другим долговым обязательствам';uk='Дохід у вигляді відсотків, отриманих за договорами позики і іншими борговими зобов`язаннями'"));
	ПризнакиПредметаРасчета.Вставить(7, НСтр("ru='Доход в виде сумм восстановленных резервов';uk='Дохід у вигляді сум відновлених резервів'"));
	ПризнакиПредметаРасчета.Вставить(8, НСтр("ru='Доход в виде безвозмездно полученного имущества (работ, услуг) или имущественных прав';uk='Дохід у вигляді безоплатно отриманого майна (робіт, послуг) або майнових прав'"));
	ПризнакиПредметаРасчета.Вставить(9, НСтр("ru='Доход в виде дохода, распределяемого в пользу налогоплательщика при его участии в простом товариществе';uk='Дохід у вигляді доходу, що розподіляється на користь платника податків при його участі в простому товаристві'"));
	ПризнакиПредметаРасчета.Вставить(10, НСтр("ru='Доход в виде дохода прошлых лет, выявленного в отчетном (налоговом) периоде';uk='Дохід у вигляді доходу минулих років, виявленого в звітному (податковому) періоді'"));
	ПризнакиПредметаРасчета.Вставить(11, НСтр("ru='Доход в виде положительной курсовой разницы';uk='Дохід у вигляді позитивної курсової різниці'"));
	ПризнакиПредметаРасчета.Вставить(12, НСтр("ru='Доход в виде основных средств и нематериальных активов, безвозмездно полученных атомными станциями';uk='Дохід у вигляді основних засобів і нематеріальних активів, безоплатно отриманих атомними станціями'"));
	ПризнакиПредметаРасчета.Вставить(13, НСтр("ru='доход в виде стоимости полученных материалов при ликвидации выводимых из эксплуатации основных средств';uk='дохід у вигляді вартості отриманих матеріалів при ліквідації основних засобів, що виводяться з експлуатації'"));
	ПризнакиПредметаРасчета.Вставить(14, НСтр("ru='Доход в виде использованных не по целевому назначению имущества, работ, услуг';uk='Дохід у вигляді використаних не за цільовим призначенням майна, робіт, послуг'"));
	ПризнакиПредметаРасчета.Вставить(15, НСтр("ru='Доход в виде использованных не по целевому назначению средств, предназначенных для формирования резервов по обеспечению безопасности производств';uk='Дохід у вигляді використаних не за цільовим призначенням засобів, призначених для формування резервів по забезпеченню безпеки виробництв'"));
	ПризнакиПредметаРасчета.Вставить(16, НСтр("ru='Доход в виде сумм, на которые уменьшен уставной (складочный) капитал (фонд) организации';uk='Дохід у вигляді сум, на які зменшений статутний капітал (фонд) організації'"));
	ПризнакиПредметаРасчета.Вставить(17, НСтр("ru='Доход в виде сумм возврата от некоммерческой организации ранее уплаченных взносов (вкладов)';uk='Дохід у вигляді сум повернення від некомерційної організації раніше сплачених внесків (вкладів)'"));
	ПризнакиПредметаРасчета.Вставить(18, НСтр("ru='Доход в виде сумм кредиторской задолженности, списанной в связи с истечением срока исковой давности или по другим основаниям';uk='Дохід у вигляді сум кредиторської заборгованості, списаної у зв`язку із закінченням терміну позовної давності або по інших підставах'"));
	ПризнакиПредметаРасчета.Вставить(19, НСтр("ru='Доход в виде доходов, полученных от операций с производными финансовыми инструментами';uk='Дохід у вигляді доходів, отриманих від операцій з похідними фінансовими інструментами'"));
	ПризнакиПредметаРасчета.Вставить(20, НСтр("ru='Доход в виде стоимости излишков материально-производственных запасов и прочего имущества, которые выявлены в результате инвентаризации';uk='Дохід у вигляді вартості надлишків матеріально-виробничих запасів і іншого майна, які виявлені в результаті інвентаризації'"));
	ПризнакиПредметаРасчета.Вставить(21, НСтр("ru='Доход в виде стоимости продукции СМИ и книжной продукции, подлежащей замене при возврате либо при списании';uk='Дохід у вигляді вартості продукції ЗМІ і книжкової продукції, що підлягає заміні при поверненні або при списанні'"));
	ПризнакиПредметаРасчета.Вставить(22, НСтр("ru='Доход в виде сумм корректировки прибыли налогоплательщика';uk='Дохід у вигляді сум коригування прибутку платника податків'"));
	ПризнакиПредметаРасчета.Вставить(23, НСтр("ru='Доход в виде возвращенного денежного эквивалента недвижимого имущества и (или) ценных бумаг, переданных на пополнение целевого капитала некоммерческой организации';uk='Дохід у вигляді поверненого грошового еквіваленту нерухомого майна та (або) цінних паперів, переданих на поповнення цільового капіталу некомерційної організації'"));
	ПризнакиПредметаРасчета.Вставить(24, НСтр("ru='Доход в виде разницы между суммой налоговых вычетов из сумм акциза и указанных сумм акциза';uk='Дохід у вигляді різниці між сумою податкових вирахувань з сум акцизу і вказаних сум акцизу'"));
	ПризнакиПредметаРасчета.Вставить(25, НСтр("ru='Доход в виде прибыли контролируемой иностранной компании';uk='Дохід у вигляді прибутку контрольованої іноземної компанії'"));
	ПризнакиПредметаРасчета.Вставить(26, НСтр("ru='Взносы на ОПС';uk='Внески на ОПС'"));
	ПризнакиПредметаРасчета.Вставить(27, НСтр("ru='Взносы на ОСС в связи с нетрудоспособностью';uk='Внески на ОСС у зв`язку з непрацездатністю'"));
	ПризнакиПредметаРасчета.Вставить(28, НСтр("ru='Взносы на ОМС';uk='Внески на ОМС'"));
	ПризнакиПредметаРасчета.Вставить(29, НСтр("ru='Взносы на ОСС от несчастных случаев';uk='Внески на ОСС від нещасних випадків'"));
	ПризнакиПредметаРасчета.Вставить(30, НСтр("ru='Пособие по временной нетрудоспособности';uk='Посібник з тимчасової непрацездатності'"));
	ПризнакиПредметаРасчета.Вставить(31, НСтр("ru='Платежи по добровольному личному страхованию';uk='Платежі по добровільному особистому страхуванню'"));
...
Функция ПараметрыОперацииФискализацииЧека() Экспорт;
	...
	ПараметрыЧека.Вставить("ОрганизацияКодПоЕДРПОУ"      ); // Строка,Необязательно - КодПоЕДРПОУ организации (Для чека ЕНВД).
	...
	ПараметрыЧека.Вставить("ТекстПодвала" , НСтр("ru='СПАСИБО ЗА ПОКУПКУ!';uk='ДЯКУЄМО ЗА ПОКУПКУ!'")); // Строка - заполняется значением по умолчанию. 
...
Функция ПараметрыФискальнойСтрокиЧека() Экспорт;
	...
	ПараметрыСтроки.Вставить("ПроцентСкидки");
	ПараметрыСтроки.Вставить("СтавкаНДССтрокой");
	...
	// Реквизиты начиная с ФФД 1.1 (Введены приказом от 9 апреля 2018 г. N ММВ-7-20/207@)
	ПараметрыСтроки.Вставить("КодСтраныПроисхожденияТовара"); // Строка,Необязательно - Цифровой код страны происхождения товара в соответствии с классификатором стран мира.
	...
	ПараметрыСтроки.Вставить("ЭтоПодакцизныйТовар");          
...
Функция КомандыРаботыСоСменами()
	...
	МассивРезультат.Добавить("OpenDay");
	МассивРезультат.Добавить("PrintZReport");
...
Функция ТребуетсяЗакрытиеСмены(СтатусСмены) Экспорт
	...
	ИначеЕсли СтатусСмены = 2 или СтатусСмены = 3 Тогда // открыта или истекла
		Возврат Истина;
	Иначе
		ТекстОшибки = НСтр("ru='Неизвестный статус смены';uk='Неизвестный статус смены'");
