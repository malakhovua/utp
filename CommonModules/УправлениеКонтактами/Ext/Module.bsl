...
Процедура ПроверитьНапоминанияПользователя(ВыбПользователь) Экспорт
	...
	Запрос.Текст = "
		...
		|		Выполнена = ЛОЖЬ И Оповещение = ИСТИНА И СрокОповещения < &ДатаСреза И 
		|НЕ EDIN) 
		|";
	...
	// + umk Если edin - задача
	// Оповещение о задачах EDI только для выписки. 
	ВыпискаДокументов = Справочники.ГруппыПользователей.ВыпискаДокументов_umk;
	
	Если Справочники.ГруппыПользователей.ПользовательВходитВГруппу(ВыпискаДокументов) Тогда
		...
		|ГДЕ
		|	Задачи.ГруппаПользователей = &ГруппаПользователей
		|	И Задачи.Выполнена = ЛОЖЬ
		|	И Задачи.Оповещение = ИСТИНА
		|	И Задачи.СрокОповещения < &ДатаСреза
		|	И Задачи.EDIN";
		
		Запрос.УстановитьПараметр("ГруппаПользователей", ВыпискаДокументов);
		Запрос.УстановитьПараметр("ДатаСреза"      , ТекущаяДата());
		
		ФормаОповещения = глЗначениеПеременной("ФормаОповещенияОЗадачах");
		ФормаОповещения.EDIN = ИСТИНА;
		ФормаОповещения.Пользователи = ВыпискаДокументов;
