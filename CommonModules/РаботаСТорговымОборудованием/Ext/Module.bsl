...
Функция ПолучитьПроверитьПараметрыДляОплатыКартой(ЭС, ФР) Экспорт
	...
	ЭС = ПолучитьЭлементТО(Перечисления.ВидыТорговогоОборудования.ЭквайринговаяСистема,
	   НСтр("ru='Необходимо выбрать эквайринговую систему';uk='Необхідно вибрати еквайрингову систему'"), НСтр("ru='Эквайринговая система не подключена!';uk='Еквайрингова система не підключена!'"));
...
Функция ПроверитьСообщитьЧтоКартаНеДисконтная(ИнформационнаяКарта) Экспорт
	...
	Если ИнформационнаяКарта.ТипКарты <> Перечисления.ТипыИнформационныхКарт.Дисконтная Тогда
		Результат = Истина;
		Предупреждение(НСтр("ru='Считанная карта не является дисконтной.';uk='Зчитана картка не є дисконтною.'"));
	Иначе
		Результат = Ложь;
	КонецЕсли;
		...
		Для Каждого ФайлОО Из СписокФайлов Цикл
			...
			Исключение
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Ошибка при обработке файла: ';uk='Помилка при обробці файлу: '") + ФайлОО.Имя + Символы.ПС
					    ...
					    + НСтр("ru='либо формат обработки не соответствует текущей версии системы.';uk='або формат обробки не відповідає поточній версії системи.'"), , , СтатусСообщения.Внимание);
				Индикатор.Значение = Индикатор.Значение + 1;
				Продолжить;
			КонецПопытки;
