	...
	Для Каждого Платеж Из РасшифровкаСделка Цикл
		...
		Если Платеж.ПроцентПредоплаты > 0 И (НЕ РасчетыВозврат=Перечисления.РасчетыВозврат.Возврат) Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Недостаточна предоплата по заказу ';uk='Недостатня передоплата по замовленню '") + Сделка + 
				Символы.ПС + Символы.Таб + НСтр("ru=' По договору ""';uk=' По договору ""'") + ДоговорКонтрагента + 
				НСтр("ru='"" требуется предоплата в размере ';uk='"" потрібна передоплата в розмірі '") + ПроцентПредоплаты + НСтр("ru='% ; Поступила предоплата  в размере ';uk='% ; Надійшла передоплата в розмірі '") 
...
Процедура УстановитьСтатьюДДСПоУмолчанию(СтрокаПлатеж,ВидОперацииДокумент) Экспорт;
	...
	ИначеЕсли ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента.ОсновнаяСтатьяДвиженияДенежныхСредств) Тогда
...
Процедура УстановитьНадписьСделкиПлатеж(СтрокаПлатеж, ФормаДокумента, ВидОперации) Экспорт
	...
	Если Не (Не ЗначениеЗаполнено(ДоговорКонтрагента) ИЛИ ТипЗаказа="Прочие")  Тогда
		Если ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом
			...
			НадписьСделка = НСтр("ru='Заказ ';uk='Замовлення '")+?(ТипЗаказа="ЗаказПокупателя", НСтр("ru='покупателя:';uk='покупця:'"), НСтр("ru='поставщику:';uk='постачальнику:'"));

		ИначеЕсли ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам тогда

			НадписьСделка = НСтр("ru='Счет ';uk='Рахунок '")+?(ТипЗаказа="ЗаказПокупателя", НСтр("ru='покупателю:';uk='покупцю:'"), НСтр("ru='поставщика:';uk='постачальника:'"));
