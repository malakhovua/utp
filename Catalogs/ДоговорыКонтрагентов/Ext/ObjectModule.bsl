...
"Реквізит ""Відособлений облік товарів по замовленнях покупців"" не може бути змінений, елемент не записаний.'"), 
	...
	Если ЭтоНовый() и Не Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		Попытка
			ПосленийНомер = Число(УстановитьНомерДоговора());
			ПосленийНомер = ПосленийНомер + 1;
			Номер = Формат(ПосленийНомер,"ЧЦ=3; ЧВН=") + "-" + Формат(?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДата()),"ДФ=ddMMyy");
		Исключение
			Сообщить("Некорректный номер последнего договора:" + УстановитьНомерДоговора(Истина) + ". Номер не установлен.", СтатусСообщения.Важное);
		КонецПопытки;
...
КонецПроцедуры
// + umk
Функция УстановитьНомерДоговора(Сообщать = Ложь)Экспорт
	...
	Запрос.УстановитьПараметр("Дата", ?(ЗначениеЗаполнено(Дата), Дата, НачалоДня(ТекущаяДата())));
	...
	Если Выборка.Следующий() Тогда
		Если Сообщать Тогда
			Сообщить("Номер последнего договора:" + (Выборка.Номер) + " от " + Формат(Дата,"ДФ=dd.MM.yyyy"));
		КонецЕсли;
		Возврат Лев(Выборка.Номер,3)
	Иначе
		Возврат "0"
	КонецЕсли;
