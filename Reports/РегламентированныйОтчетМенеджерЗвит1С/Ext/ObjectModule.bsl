...
Функция ЗаполнитьПараметрыРаботыЗвит1С()
	...
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить(-2, НСтр("ru = 'В ""FREDO Звіт"" обнаружено более одного документа соответствующего документу информационной базы'; uk = 'В ""FREDO Звіт"" виявлено більше одного документа, що відповідає документу інформаційної бази'"));
	...
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить(-2, НСтр("ru = 'В ""FREDO Звіт"" обнаружено более одной выписки соответствующей документу информационной базы'; uk = 'В ""FREDO Звіт"" виявлено більше одного витягу що відповідає документу інформаційній базі'")); // Несуществующее состояние для входящих 
...
// Функция возвращает таблицу значений с соответствия показателей в базе и FREDO Звіт
...
// Функция возвращает имя поля в таблицах  
//
...
Функция ПолучитьЗначениеПоказателя1С(Данные, ИмяТаблицы,ИмяПоказателя)
	...
	// Если показательв базе не задан, он может быть только вычислен
...
Функция Инициализация(ВыводитьСообщенияОбОшибках) Экспорт
	...
	// Установим представление для FREDO Звіт
	Попытка
...
Функция ПроверитьКлючевыеПараметрыПередПовторнойВыгрузкой(ОтчетЗвит1С, ТекстОшибки, ИмяСхемыЗвит1С, ТипПериода, НомерВПериоде, Год, КодПоЕДРПОУ);
	...
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		ТекстОшибки = НСтр("ru = 'Обнаружен отчет ""FREDO Звіт"" соответствующий выгружаемому из информационной базы документу.
                           |Изменены ключевые параметры, выгрузка отменена.'; uk = 'Виявлено звіт ""FREDO Звіт"" відповідний документу, що вивантажується з інформаційної бази.
...
Функция ВыгрузитьДокументВЗвит1С(ДокументСсылка, ПараметрыПакетнойВыгрузки = Неопределено) Экспорт
	...
	// Определим рабочую строку из полной таблицы соответствий отчетов базы и FREDO Звіт	
	ТекОтчет = РегламентированнаяОтчетность.РеглОтчеты(ИмяОтчета);
	Если ТекОтчет = Неопределено Тогда
...
Функция ВыгрузитьПервичныйДокументВЗвит1С(ДокументСсылка, ПараметрыПакетнойВыгрузки = Неопределено) Экспорт
	...
	// Определим рабочую строку из полной таблицы соответствий отчетов базы и FREDO ДокМен	
	ТекОтчет = РегламентированнаяОтчетность.РеглОтчеты(ИмяОтчета);
	Если ТекОтчет = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось открыть отчет ""'; uk = 'Не вдалося відкрити звіт ""'")+ИмяОтчета+"""";
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина, Истина);
...
Функция СформироватьТаблицыДляЗапросаСостоянияРегистрации(МассивДокументов, ДляПолученияСостоянияРегистрации)
	...
	Для каждого Документ Из МассивДокументов Цикл
		...
		Если СтрокаРезультат.ЭтоИсходящийДокумент Тогда
			...
			// Для проверки ключевых полей в базе и FREDO Звіт
...
// 
Функция ПолучитьСостояниеРегистрацииДокументов(МассивДокументов, СтатусРег) Экспорт
	...
	ТаблицаРезультат.Колонки.Добавить("Звит1С_СовпадаютКлючевыеПоля");           // только исх. Если документ не выгружен то Неопределено, иначе Булево - ложь в случае когда документ выгружен из базы и в FREDO Звіт различаются ключевые поля
	...
	ТаблицаРезультат.Колонки.Добавить("Звит1С_Состояние");                       // Если ошибка в базе, то Звит1С_Состояние = Неопределено, иначе число возвращенное из FREDO Звіт
	...
	Для каждого СтрокаРезультат Из ТаблицаРезультат Цикл
		...
		Если ФлагОтладки и СтатусРег <> 10 Тогда
			СтрокаРезультат.Звит1С_Состояние       = СтатусРег;		
		Иначе	
		...
		КонецЕсли;
		СтрокаРезультат.Звит1С_ДокументЗарегистрирован = (СтрокаРезультат.Звит1С_Состояние = 4) или (СтрокаРезультат.Звит1С_Состояние = 6);
		
		Если ФлагОтладки и СтатусРег <> 10 Тогда
			Звит1С_ДатаРегистрации  = ТекущаяДата();
		Иначе	
