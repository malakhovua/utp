...
Процедура УстановитьНачальныеНастройкиПоМетаданнымРегистра(ИмяРегистра)
	...
	Для каждого Ресурс Из МетаданныеРегистра.Ресурсы Цикл
		...
		Иначе
			...
			мСтруктураПредставлениеПолей.Вставить(Ресурс.Имя + "НачальныйОстаток", Ресурс.Представление() + НСтр("ru = ' (нач. ост.)'; uk = ' (поч. зал.)'"));
			мСтруктураПредставлениеПолей.Вставить(Ресурс.Имя + "Приход", Ресурс.Представление() + НСтр("ru = ' (приход)'; uk = ' (прибуток)'"));
			мСтруктураПредставлениеПолей.Вставить(Ресурс.Имя + "Расход", Ресурс.Представление() + НСтр("ru = ' (расход)'; uk = ' (витрата)'"));
			мСтруктураПредставлениеПолей.Вставить(Ресурс.Имя + "КонечныйОстаток", Ресурс.Представление() + НСтр("ru = ' (кон. ост.)'; uk = ' (кін. зал.)'"));
			мСтруктураПредставлениеПолей.Вставить(Ресурс.Имя + "Оборот", Ресурс.Представление() + НСтр("ru = ' (оборот)'; uk = ' (оборот)'"));
...
Процедура ДобавитьВТекстЗапросаСвойстваИКатегории(ТекстЗапроса, ТаблицаПолей = Неопределено, СтруктураПредставлениеПолей = Неопределено, СоответствиеНазначений = Неопределено, Параметры = Неопределено, СоответствиеСвойствКатегорийИИзмерений = Неопределено, СтруктураЗаменять = Неопределено) Экспорт
	...
	Для каждого Поле из ТаблицаПолей Цикл
		...
		Если СоответствиеДобавленныеКатегории.Получить(Поле.ПутьКДанным) = Неопределено Тогда
			...
			СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "КатегорииКатегория", Поле.Представление + НСтр("ru = ' (категории)'; uk = ' (категорії)'"));
			СоответствиеНазначений.Вставить(Поле.Представление + НСтр("ru = ' (категории)'; uk = ' (категорії)'"), Поле.Назначение);
...
Процедура ДобавитьВТекстЗапросаДанныеОФизлице(ТекстЗапроса, ТаблицаПолей = Неопределено, СтруктураПредставлениеПолей = Неопределено, Параметры = Неопределено, СоответствиеДанныхОФизлицеИИзмерений = Неопределено, СтруктураЗаменять = Неопределено) Экспорт
	...
	Для каждого Поле из ТаблицаПолей Цикл
		...
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ФиоПолное", НСтр("ru = 'ФИО (полностью)'; uk = 'ПІБ (повністю)'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ФиоКраткое", НСтр("ru='Фамилия И. О.';uk='Прізвище І. Б.'"));
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Возраст", НСтр("ru = 'Возраст'; uk = 'Вік'"));
