...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|		"""+НСтр("ru='Норма час.';uk='Норма год.'", КодЯзыкаПечать)+"""   КАК ИмяВидаРасчета,
	|		NULL КАК ПриоритетВидаРасчета,
	|		""А"" КАК Группа,
	|		2 КАК ПриоритетГруппы,
	|		СУММА(ГрафикиРаботыПоВидамВремени.ОсновноеЗначение) КАК Результат
	|	ИЗ
	|		РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени,
	|		РегистрСведений.РаботникиОрганизаций.СрезПоследних(КОНЕЦПЕРИОДА(&Период, МЕСЯЦ), ) КАК РаботникиОрганизацийСрезПоследних
	|	ГДЕ
	|		ГрафикиРаботыПоВидамВремени.Дата <= КОНЕЦПЕРИОДА(&Период, МЕСЯЦ)
	|		И ГрафикиРаботыПоВидамВремени.Дата >= НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	...
	|	ГДЕ
	|		РабочееВремяРаботниковОрганизаций.Период >= НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|		И РабочееВремяРаботниковОрганизаций.Период <= КОНЕЦПЕРИОДА(&Период, МЕСЯЦ)
