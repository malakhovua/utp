...
Процедура ДобавитьВТекстЗапросаСвойстваИКатегории(ТекстЗапроса, ТаблицаПолей = Неопределено, СтруктураПредставлениеПолей = Неопределено, СоответствиеНазначений = Неопределено, Параметры = Неопределено, СоответствиеСвойствКатегорийИИзмерений = Неопределено, СтруктураЗаменять = Неопределено) Экспорт
	...
	Для каждого Поле из ТаблицаПолей Цикл
		...
		Пока СвойстваОбъектов.Следующий() Цикл
			...
			СтруктураПредставлениеПолей.Вставить(ИмяСвойства + "Значение", СвойстваОбъектов.Наименование + НСтр("ru = ' (св-во '; uk = ' (вл-ть '") + Поле.Представление + ")");
			СоответствиеНазначений.Вставить(СвойстваОбъектов.Наименование + НСтр("ru = ' (св-во '; uk = ' (вл-ть '") + Поле.Представление + ")", СвойстваОбъектов.Ссылка);
		...
		Пока КатегорииОбъектов.Следующий() Цикл
			...
			СтруктураПредставлениеПолей.Вставить(ИмяКатегории + "Категория", КатегорииОбъектов.Наименование + НСтр("ru = ' (категория '; uk = ' (категорія '") + Поле.Представление + ")");
...
Процедура УстановитьПредставленияОтборов() Экспорт
	...
	Для Индекс = 1 по КоличествоОстатки Цикл
		...
		ИначеЕсли ПустаяСтрока(Склады.Получить(Индекс - 1).Представление) Тогда
			...
			мСтруктураПредставлениеПолей.Вставить("Остатки_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0"), НСтр("ru = 'Склад: '; uk = 'Склад: '") + Склады.Получить(Индекс - 1).Склад.Наименование);
	...
	Для Индекс = 1 по КоличествоЦены Цикл
		...
		ИначеЕсли ПустаяСтрока(ТипыЦен.Получить(Индекс - 1).Представление) Тогда
			...
			мСтруктураПредставлениеПолей.Вставить("Цена_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0"), НСтр("ru = 'Цена: '; uk = 'Ціна: '")  + ТипыЦен.Получить(Индекс - 1).ТипЦен.Наименование);
	...
	Для Индекс = 1 по КоличествоПродажи Цикл
		...
		Иначе
			...
			Если ПериодичностьПродаж = 0 Тогда
				...
				мСтруктураПредставлениеПолей.Вставить("Продажи_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0"), НСтр("ru = 'Продажи: с '; uk = 'Продажі: з '") + Формат(ПериодНачало, мСтруктураФорматаПолей["Период" + Строка(ПериодичностьПродаж)]) +  " по " + Формат(ПериодОкончание, мСтруктураФорматаПолей["Период" + Строка(ПериодичностьПродаж)]));
				
			Иначе
				
				мСтруктураПредставлениеПолей.Вставить("Продажи_" + Формат(Индекс, "ЧЦ=2; ЧВН=; ЧГ=0"), НСтр("ru = 'Продажи: '; uk = 'Продажі: '") + Формат(ПериодНачало, мСтруктураФорматаПолей["Период" + Строка(ПериодичностьПродаж)]));
	...
	Для Индекс = 1 по КоличествоПоля Цикл
		...
		Если Индекс <= ПостроительОтчета.ВыбранныеПоля.Количество() Тогда
			...
			Иначе
				...
				Если Найти(Представление, НСтр("ru = '(св-во'; uk = '(вл-ть'")) > 0 Тогда
					
					Представление = СокрЛП(Лев(Представление, Найти(Представление, НСтр("ru = '(св-во'; uk = '(вл-ть'")) - 1));

				ИначеЕсли Найти(Представление, НСтр("ru = '(категория'; uk = '(категорія'")) > 0 Тогда
					
					Представление = СокрЛП(Лев(Представление, Найти(Представление, НСтр("ru = '(категория'; uk = '(категорія'")) - 1));
...
мСтруктураФорматаПолей.Вставить("Период2", НСтр("ru = 'ДФ = ''""Неделя с"" дд.ММ.гггг '''; uk = 'ДФ = ''""Тиждень з"" дд.ММ.гггг '''"));
мСтруктураФорматаПолей.Вставить("Период3", НСтр("ru = 'ДФ = ''""Декада с"" дд.ММ.гггг '''; uk = 'ДФ = ''""Декада з"" дд.ММ.гггг '''"));
мСтруктураФорматаПолей.Вставить("Период4", НСтр("ru = 'ДФ = ''ММММ гггг ""г.""'''; uk = 'ДФ = ''ММММ гггг ""р.""'''"));
мСтруктураФорматаПолей.Вставить("Период5", НСтр("ru = 'ДФ = ''к ""квартал"" гггг ""г.""'''; uk = 'ДФ = ''к ""квартал"" гггг ""р.""'''"));
мСтруктураФорматаПолей.Вставить("Период6", НСтр("ru = 'ДФ = ''""Полугодие с"" дд.ММ.гггг'''; uk = 'ДФ = ''""Півріччя з"" дд.ММ.гггг'''"));
мСтруктураФорматаПолей.Вставить("Период7", НСтр("ru = 'ДФ = ''гггг ""г.""'''; uk = 'ДФ = ''гггг ""р.""'''"));
