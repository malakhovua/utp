				...
				Если СтрЧислоВхождений(СтрокаПолей.ИндПоля, ";") = 0 Тогда
					...
					ИначеЕсли ТипОбъекта = "КонтактныеЛицаКонтрагентов"
						...
						Если ПреобразованноеЗначениеФайла = Неопределено Тогда
							Сообщить(НСтр("ru='Установлено несоответствие типов полей. ИБ - ';uk='Встановлена невідповідність типів полів. ІБ - '") + СокрЛП(СтрокаПолей.НазначениеРеквизита1С) + НСтр("ru=', файл - ';uk=', файл - '") + СокрЛП(СтрокаПолей.ИмяРеквизита));
							Продолжить;
						КонецЕсли;
