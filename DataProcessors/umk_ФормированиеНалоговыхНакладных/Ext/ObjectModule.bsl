...
"Не указана схема налогового учета по таре, либо в схеме не указан Момент определения базы НДС по продажам!", ВыборкаСделка.Контрагент, ВыборкаСделка.ДоговорКонтрагента); 
							Иначе
								ТекстСообщения = 
										  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Контрагент: %1, Договор: %2"
"Не указана схема налогового учета, либо в схеме не указан Момент определения базы НДС по продажам!", ВыборкаСделка.Контрагент, ВыборкаСделка.ДоговорКонтрагента); 
							КонецЕсли;	
...
"Нет возможности определить параметры (ставка, счет учета) налоговых обязательств. Общая сумма обязательств указана в колонке ""Аванс""!", ВыборкаДоговора.Контрагент, ВыборкаДоговора.ДоговорКонтрагента, Формат(ВыборкаДоговора.Дата,"ДФ=dd.MM.yyyy")); 
...
Процедура ДобавитьСтрокиВТабличнуюЧастьНалоговогоДокумента(Документ, ИмяТабличнойЧасти, СтрокиТабличнойЧастиОбработки, Данные)
	...
	Для каждого СтрокаТЧОбработки Из СтрокиТабличнойЧастиОбработки Цикл
		...
		// + umk
		СтрокаТЧДокумента.НомерСтрокиНН = НомерСтрокиНН;
		НомерСтрокиНН = НомерСтрокиНН + 1;
		// - umk		
	КонецЦикла;
