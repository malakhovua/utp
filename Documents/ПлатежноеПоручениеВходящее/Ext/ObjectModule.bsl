...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента,РежимПроведения, Отказ, Заголовок)
	...
	Если ЕстьРасчетыСКонтрагентами ИЛИ ЕстьРасчетыПоКредитам ИЛИ ЕстьРасчетыПоПлатежнымКартам Тогда
		...
		ИначеЕсли ЕстьРасчетыПоПлатежнымКартам тогда
			СодержаниеПроводки =  НСтр("ru='Расчеты по платежным картам и банковским кредитам';uk='Розрахунки за платіжними картками та банківськими кредитами'",Локализация.КодЯзыкаИнформационнойБазы());
			РеестрПлатежей.ЗагрузитьКолонку(РеестрПлатежей.ВыгрузитьКолонку("СчетОплаты"),"СчетАванса");
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ПоступлениеСредствОтФСС Тогда
		...
		Пока ВыборкаДляПроводок.Следующий() Цикл
			...
			Проводка.Содержание = НСтр("ru='Поступление денежных средств от ФСС';uk='Надходження коштів від ФСС'",Локализация.КодЯзыкаИнформационнойБазы());
