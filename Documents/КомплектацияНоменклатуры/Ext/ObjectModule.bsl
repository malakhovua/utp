...
Функция ПодготовитьТаблицуКомплектовДляРегл(ТаблицаПоКомплектующим, СтруктураШапкиДокумента)
	...
	Для Каждого Строка Из ТаблицаКомплектов Цикл
		...
		Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.Разукомплектация 
			И ЗначениеЗаполнено(СкладМатериалов) Тогда
			Строка.КорСубконтоСписанияБУ2 = СкладМатериалов;
			Строка.Склад = Склад;			
		КонецЕсли;// + umk
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок);
	...
	// + umk 
	Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.Разукомплектация Тогда 
		ДополнитьСкладМатериаловВТаблицыДокумента(Новый Структура("ТаблицаПоКомплектующим",ТаблицаПоКомплектующим), "ТаблицаПоКомплектующим");
	КонецЕсли;
	// - umk
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок)
	...
	Если ОтражатьВУправленческомУчете Тогда
		...
		Если НЕ Отказ Тогда
			...
			// + umk
			ДополнитьСкладМатериаловВТаблицыДокумента(ТаблицыДанныхДокумента,"ТаблицаПоКомплектующим");
			// - umk
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок)
	...
	Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.Комплектация Тогда
		...
		СодержаниеПроводок = НСтр("ru='Комплектация';uk='Комплектація'",Локализация.КодЯзыкаИнформационнойБазы());
	Иначе
		ТаблицаТоваров = ПодготовитьТаблицуКомплектовДляРегл(ТаблицаПоКомплектующим, СтруктураШапкиДокумента);
		СодержаниеПроводок = НСтр("ru='Разукомплектация';uk='Розукомплектація'",Локализация.КодЯзыкаИнформационнойБазы());
	КонецЕсли;
...
Процедура ДвиженияПоСписаннымТоварам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ВестиПартионныйУчетПоСкладам Тогда 
		...
		// + umk Уберем и оставим склад как есть
		ДополнитьСкладМатериаловВТаблицыДокумента(Новый Структура("ТаблицаДвижений",ТаблицаДвижений), "ТаблицаДвижений");
		// - umk
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//  + umk
	ДополнитьСкладМатериаловКомплектующих(ТаблицаПоКомплектующим);
	//  - umk 
	...
	Если СтруктураШапкиДокумента.Услуга ИЛИ СтруктураШапкиДокумента.Набор Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Комплектуемая номенклатура не может быть услугой или набором!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете И СтруктураШапкиДокумента.СчетУчетаБУ.Забалансовый Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Комплектуемая номенклатура не может учитываться на забалансовом счете!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДополнитьСкладМатериаловКомплектующих(ТаблицаПоКомплектующим) 
	
	Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.Разукомплектация Тогда
		Возврат
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СкладМатериалов) Тогда
		
		Для Каждого СтрокаТабличнойЧасти Из ТаблицаПоКомплектующим Цикл 
									
			Если СтрокаТабличнойЧасти.СчетУчетаБУ = ПланыСчетов.Хозрасчетный.ГотоваяПродукция Тогда
				Продолжить // Склад меняем только у материалов
			КонецЕсли;
			
			СтрокаТабличнойЧасти.Склад = СкладМатериалов;
...
Процедура ДополнитьСкладМатериаловВТаблицыДокумента(ТаблицыДанныхДокумента, ИмяТаблицы)
	
	Если ЗначениеЗаполнено(СкладМатериалов) Тогда
				
		Для Каждого СтрокаТабличнойЧасти Из ТаблицыДанныхДокумента[ИмяТаблицы] Цикл
			...
			Если Не СтрокаТабличнойЧасти.Номенклатура.ВидНоменклатуры = Константы.umk_ВидНоменклатурыПродукция.Получить() Тогда
				СтрокаТабличнойЧасти.Склад = СкладМатериалов;
			КонецЕсли;
		КонецЦикла;
