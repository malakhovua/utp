...
Процедура ДвиженияРегистровВыпускаПродукцииУслугРегл(СтруктураШапкиДокумента, ТаблицаПоУслугам, Отказ)
	...
	Для каждого СтрокаУслуги Из ТаблицаПоУслугам Цикл
		...
		Проводка.Содержание	  = НСтр("ru='Затраты от реализации услуг перераб.дав.сырья в план.ценах';uk='Витрати від послуг перероб.дав.сировини в план.ценах'",Локализация.КодЯзыкаИнформационнойБазы());
		Проводка.НомерЖурнала = "";
