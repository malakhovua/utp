...
Перем СуммаВыручкиККМ, СуммаВыручкиККМВозврат;  
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоКомплектам, ТаблицаПоПлатежнымКартам, ТаблицаПоБанковскимКредитам, Отказ, Заголовок)
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
		...
		Если СтрокаТаблицы.РасчетыВозврат = Перечисления.РасчетыВозврат.Расчеты Тогда
			...
			Проводка.Содержание   = НСтр("ru='Доход от продаж в рознице';uk='Дохід від продажу в роздріб'",Локализация.КодЯзыкаИнформационнойБазы());
			...
			Если Дата >= глЗначениеПеременной("ДатаНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ") Тогда
				
				СуммаВыручкиККМ = СуммаВыручкиККМ + СтрокаТаблицы.ПроводкиСуммаСНДСРегл + СтрокаТаблицы.ПроводкиСуммаАкцизногоНалогаРегл;
		...
		Иначе
			...
			Проводка.Организация     = СтруктураШапкиДокумента.Организация;
			Проводка.Сумма           = - СуммаВыручки;
			Проводка.Содержание   = НСтр("ru='Вычет из доходов от продаж в рознице (возврат)';uk='Видрахування з доходів від продажу в роздріб (повернення)'",Локализация.КодЯзыкаИнформационнойБазы());
			Проводка.НомерЖурнала    = "";
			...
			Если Дата >= глЗначениеПеременной("ДатаНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ") Тогда
				...
				СуммаВыручкиККМВозврат = СуммаВыручкиККМВозврат + СтрокаТаблицы.ПроводкиСуммаСНДСРегл + СтрокаТаблицы.ПроводкиСуммаАкцизногоНалогаРегл;
	...
	Если Дата >= глЗначениеПеременной("ДатаНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ") Тогда
		...
		Если СуммаВыручкиККМ <> 0 Тогда
			
			ПроводкаККМ = ПроводкиБУ.Добавить();
			
			ПроводкаККМ.Период          = Дата;
			ПроводкаККМ.Активность      = Истина;
			ПроводкаККМ.Организация     = СтруктураШапкиДокумента.Организация;
			ПроводкаККМ.Сумма           = СуммаВыручкиККМ;
			ПроводкаККМ.Содержание   	= НСтр("ru='Доход от продаж в рознице (ККМ)';uk='Дохід від продажу в роздріб (ККМ)'",Локализация.КодЯзыкаИнформационнойБазы());
			ПроводкаККМ.НомерЖурнала    = "";
			ПроводкаККМ.СчетКт          = СчетОтраженияДоходов;
			ПроводкаККМ.СчетДт   	    = ПланыСчетов.Хозрасчетный.ПрочиеДенежныеСредстваВКассе;
			ОбщегоНазначения.УстановитьСубконто(ПроводкаККМ.СчетДт, ПроводкаККМ.СубконтоДт, "СтатьиДвиженияДенежныхСредств", СтатьяДвиженияДенежныхСредств);
			
		КонецЕсли;
		
		Если СуммаВыручкиККМВозврат <> 0 Тогда
			...
			ПроводкаККМ.Активность      = Истина;
			ПроводкаККМ.Организация     = СтруктураШапкиДокумента.Организация;
			ПроводкаККМ.Сумма           = - СуммаВыручкиККМВозврат;
			ПроводкаККМ.Содержание   	= НСтр("ru='Вычет из доходов продаж в рознице (ККМ)';uk='Вирахування з доходів продажу в роздріб (ККМ)'",Локализация.КодЯзыкаИнформационнойБазы());
			ПроводкаККМ.НомерЖурнала    = "";
			ПроводкаККМ.СчетДт          = СчетОтраженияВычетаИзДоходов;
			ПроводкаККМ.СчетКт   	    = ПланыСчетов.Хозрасчетный.ПрочиеДенежныеСредстваВКассе;
			ОбщегоНазначения.УстановитьСубконто(ПроводкаККМ.СчетКт, ПроводкаККМ.СубконтоКт, "СтатьиДвиженияДенежныхСредств", СтатьяДвиженияДенежныхСредств);
...
Процедура ДвиженияПоРаспределениюОплат(СтруктураШапкиДокумента, ТаблицаПоПлатежнымКартам, ТаблицаПоБанковскимКредитам, Отказ, Заголовок)
	...
	Для Каждого СтрокаКарта из  ТаблицаПоПлатежнымКартам Цикл
    		
    	Проводка = ПроводкиБУ.Добавить();
    	
        Проводка.Период      = ДатаДока;
    	Проводка.Организация = СтруктураШапкиДокумента.Организация;
    	Проводка.Сумма       = СтрокаКарта.Сумма;
		Проводка.Содержание  = НСтр("ru='Оплата платежными картами';uk='Оплата платіжними картками'",Локализация.КодЯзыкаИнформационнойБазы());
		...
		Если СтрокаКарта.Сумма > 0 Тогда
	...
	Если Дата >= глЗначениеПеременной("ДатаНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ") Тогда
		...
		Для каждого СтрокаКарта Из ТаблицаПоПлатежнымКартам Цикл     
			...
			Если СтрокаКарта.Сумма > 0 Тогда  
				СуммаВыручкиККМ = СуммаВыручкиККМ - СтрокаКарта.Сумма;
			Иначе
				СуммаВыручкиККМВозврат = СуммаВыручкиККМВозврат - СтрокаКарта.Сумма;
			КонецЕсли;
    ...
    Для каждого СтрокаКредита Из ТаблицаПоБанковскимКредитам Цикл
		...
		Проводка.Содержание  = НСтр("ru='Оплата банковским кредитом';uk='Оплата банківським кредитом'",Локализация.КодЯзыкаИнформационнойБазы());
		...
		Если Дата >= глЗначениеПеременной("ДатаНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ") Тогда
			...
			Если СтрокаКредита.Сумма > 0 Тогда   
				СуммаВыручкиККМ = СуммаВыручкиККМ - СтрокаКредита.Сумма;
			Иначе 
				СуммаВыручкиККМВозврат = СуммаВыручкиККМВозврат - СтрокаКредита.Сумма;
			КонецЕсли; 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЕстьБезналичныеРасчеты Тогда
		СуммаПродажБезнал = ТаблицаПоПлатежнымКартам.Итог("Сумма") + ТаблицаПоБанковскимКредитам.Итог("Сумма");
		
		Если (СуммаДокумента < СуммаПродажБезнал и СуммаПродажБезнал > 0 и СуммаДокумента > 0) или
			 (СуммаДокумента > СуммаПродажБезнал и СуммаПродажБезнал < 0 и СуммаДокумента < 0) Тогда
