	...
	Если    (СкладОтправитель.ВидСклада = Перечисления.ВидыСкладов.Оптовый) 
		...
		Если НЕ СкладПолучатель.ВидСклада = Перечисления.ВидыСкладов.Оптовый 
		   И НЕ (     СкладПолучатель.ВидСклада = Перечисления.ВидыСкладов.НТТ 
			...
			УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров(Ссылка, ТаблицаПоТоварам, , Ложь, СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете, , , Отказ, ,НСтр("ru='Перемещение ТМЦ';uk='Переміщення ТМЦ'",Локализация.КодЯзыкаИнформационнойБазы()));
			...
			Для Каждого Партия Из Строка.СписанныеПартииБУ Цикл
				...
				Если    Сумма - ПокупнаяЦена <> 0
					...
					Иначе // АТТ или СНТТ
						...
						Проводка.Период      = Дата;
						Проводка.Организация = Организация;
						Проводка.Содержание  = НСтр("ru='Торговая наценка (корректировка продажной стоимости на стоимость наценки)';uk='Торгова націнка (коригування продажної вартості на вартість націнки)'",Локализация.КодЯзыкаИнформационнойБазы());
						...
						Проводка.Период      = Дата;
						Проводка.Организация = Организация;
						Проводка.Содержание  = НСтр("ru='Торговая наценка (списание наценки при перемещении)';uk='Торгова націнка (списання націнки при переміщення)'",Локализация.КодЯзыкаИнформационнойБазы());
				...
				Если (ПолучательАТТ ИЛИ ПолучательСНТТ) Тогда
					...
					Если НЕ СуммаДооценки = 0 Тогда
						...
						Проводка.Период      = Дата;
						Проводка.Организация = Организация;
						Проводка.Содержание  = НСтр("ru='Торговая наценка (донаценка на складе-получателе)';uk='Торгова націнка (донацінка на складі-одержувачі)'",Локализация.КодЯзыкаИнформационнойБазы());
...
"Не списано %3 %4 товара %5, рахунок обліку %6'"), Учет, СтрокаТаблицы.НомерСтроки, (СтрокаТаблицы.Количество - СтрокаТаблицы.КоличествоНоминальное), СтрокаТаблицы.Номенклатура.БазоваяЕдиницаИзмерения, СтрокаТаблицы.Номенклатура, СтрокаТаблицы.СчетУчетаЗабалансовыйБУ), "");
			...
			Иначе		
				...
				Проводка.Содержание   = НСтр("ru='Перемещение БСУ';uk='Переміщення БСО'",Локализация.КодЯзыкаИнформационнойБазы());
