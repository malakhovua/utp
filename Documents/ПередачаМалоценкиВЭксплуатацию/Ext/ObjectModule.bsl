...
Процедура ДвиженияПоРегистрамБухРегл(СтруктураШапкиДокумента, ТаблицаПоМалоценке, Отказ, Заголовок)
	...
	УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров(Ссылка, ТаблицаПоМалоценке, , Ложь, СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете, , , Отказ, , НСтр("ru='Передача малоценки в эксплуатацию';uk='Передача малоцінки в експлуатацію'",Локализация.КодЯзыкаИнформационнойБазы()));
	...
	Для Каждого СтрокаТЧ Из ТаблицаПоМалоценке Цикл
		...
		Проводка.Содержание  = НСтр("ru='Передача малоценки в эксплуатацию';uk='Передача малоцінки в експлуатацію'",Локализация.КодЯзыкаИнформационнойБазы());
	...
	Если ТабАмортизации.Количество() > 0 Тогда
		...
		УправлениеМалоценнымиАктивами.ПолучитьРаспределениеПогашенияСтоимости(ЭтотОбъект,
												...
												НомерЖурнала,
												НСтр("ru='Списание стоимости малоценки';uk='Списання вартості малоцінки'",Локализация.КодЯзыкаИнформационнойБазы()));
