...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПокупкаПродажаВалюты.ПокупкаВалюты Тогда
		...
		Если СуммаПенсионный > 0 Тогда
			...
			Проводка.Период      = Дата;
			Проводка.Организация = Организация;
			Проводка.Содержание  = НСтр("ru='Начисление сбора в ПФ';uk='Нарахування збору до ПФ'",Локализация.КодЯзыкаИнформационнойБазы());
		...
		Если СуммаКР > 0 Тогда
			...
			Проводка.Период      = Дата;
			Проводка.Организация = Организация;
			Проводка.Содержание  = НСтр("ru='Расходы: разница курсов покупки и НБУ';uk='Витрати: різниця курсів купівлі та НБУ'",Локализация.КодЯзыкаИнформационнойБазы());
		...
		ИначеЕсли СуммаКР < 0 Тогда
			...
			Проводка.Период      = Дата;
			Проводка.Организация = Организация;
			Проводка.Содержание  = НСтр("ru='Доходы: разница курсов покупки и НБУ';uk='Доходи: різниця курсів купівлі та НБУ'",Локализация.КодЯзыкаИнформационнойБазы());
