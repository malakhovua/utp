	...
	Иначе 
		...
		Для Каждого ТекСтрокаУслуги Из ТаблицаУслуги Цикл
			...
			Если ТекСтрокаУслуги.Цена * ТекСтрокаУслуги.Количество <> 0 Тогда
				...
				ПроцентСкидки = (1 - ТекСтрокаУслуги.Сумма/ОбщегоНазначения.ОкруглитьСУчетомПогрешности(ТекСтрокаУслуги.Цена*ТекСтрокаУслуги.Количество,2)) * 100 ;
...
Функция ПечатьТТН()
	...
	ОбработкаПечати = Обработки.umk_ПечатьТТН.ПолучитьФорму("Форма");
	// - umk
    
	ОбработкаПечати.Документ       = ЭтотОбъект.Ссылка;
	ОбработкаПечати.ПунктРазгрузки = ?(ЗначениеЗаполнено( umk_Регион), umk_Регион, АдресДоставки);
	// + umk
	КодЯзыкаПечать = "uk";	
	СведенияОПоставщике = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Организация, Дата,,,КодЯзыкаПечать);

	ОбработкаПечати.Перевозчик     = Организация;
	ОбработкаПечати.ПунктПогрузки  = СведенияОПоставщике.ЮридическийАдрес;
	ОбработкаПечати.Дата           = Дата;
    // - umk
...
Функция ПечатьДляВыгрузкиВ1СЗвит(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Ложь) Экспорт
	...
	Если ТолькоФормироватьСтруктуруПоказателейПечатнойФормы Тогда
		...
		|			ТОГДА 0
		|		КОГДА (ТЧ.Сумма-(ТЧ.Цена * ТЧ.Количество)) <= 0.005 и (ТЧ.Сумма-(ТЧ.Цена * ТЧ.Количество)) > -0.005
		|			ТОГДА 0
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |	Цена 			КАК Цена,
						...
						|			ТОГДА 0
						|		КОГДА (Сумма-(Цена * Количество)) <= 0.005 и (Сумма-(Цена * Количество)) > -0.005
						|			ТОГДА 0
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			КоличествоТоварыУслуг = КоличествоТоварыУслуг + 1;
		КонецЦикла;
		СтруктураПоказателей.ПервичныйДокумент.Вставить("КоличествоНаименований", КоличествоТоварыУслуг);
...
//
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, НепосредственнаяПечать = Ложь, ПакетнаяПечать = Ложь) Экспорт
	...
	// + umk
	Если ИмяМакета = "ПакетнаяПечать" Тогда
		Если ЗначениеЗаполнено(Контрагент) И Не Контрагент.ПечатныеФормы.Количество() = 0  Тогда
			Для Каждого ПечатнаяФорма Из Контрагент.ПечатныеФормы Цикл
				//ПФ не для РеализацииТоваровУслуг
				Если Не ПечатнаяФорма.СсылкаОбъекта.Ссылка.Метаданные().Имя = "РеализацияТоваровУслуг"
					Тогда
					Продолжить
				КонецЕсли;
				Если ЗначениеЗаполнено(ПечатнаяФорма.Обработка) Тогда
					//Если это не внешняя ПФ тогда
					Если ТипЗнч(ПечатнаяФорма.Обработка) = Тип("Строка") Тогда
						Если ПечатнаяФорма.Обработка = "ПакетнаяПечать" Тогда
							Продолжить
						КонецЕсли;
						
						Если      ПечатнаяФорма.Обработка = "Накладная" Тогда
							ТабДокумент = ПечатьДокумента();
						ИначеЕсли ПечатнаяФорма.Обработка = "Акт" Тогда
							ТабДокумент = ПечатьАктаОбОказанииУслуг();
						ИначеЕсли ПечатнаяФорма.Обработка = "1Т" Или ПечатнаяФорма.Обработка = "_1Т" Тогда
							ТабДокумент = ПечатьТТН();
						ИначеЕсли ПечатнаяФорма.Обработка = "СерийныеНомера" Тогда
							ТабДокумент = УчетСерийныхНомеров.ПечатьСерийныхНомеров(Ссылка, "Товары");
						ИначеЕсли ПечатнаяФорма.Обработка = "Бланк" Тогда
							ТабДокумент = ПечатьБланк(НаПринтер);
						ИначеЕсли ПечатнаяФорма.Обработка = "РасходСРозничныхСкладовВЦенахАТТ" Тогда
							ТабДокумент = ЗаполнениеДокументов.НапечататьПриходРасходСРозничныхСкладовВЦенахАТТ(Ссылка, мВалютаРегламентированногоУчета, Ложь);
						ИначеЕсли ПечатнаяФорма.Обработка = "ДляВыгрузкиВ1СЗвит" Тогда
							ТабДокумент = ПечатьДляВыгрузкиВ1СЗвит();
						КонецЕсли;
					...
					КонецЕсли;
					//Если это внешняя ПФ тогда	
					Если Не ПечатнаяФорма.Обработка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма
						Тогда
						Продолжить
					КонецЕсли;
					Расшифровка = Новый Структура();
					Расшифровка.Вставить("ВидПечатнойФормы", ПечатнаяФорма.Обработка.ВидОбработки);
					Расшифровка.Вставить("ЗаменяемаяПечатнаяФорма", Неопределено);
					Расшифровка.Вставить("НомерСтроки", 1);
					Расшифровка.Вставить("СсылкаНаВнешнююОбработку",ПечатнаяФорма.Обработка);
					ТабДокумент = УниверсальныеМеханизмы.НапечататьВнешнююФорму(Ссылка, Расшифровка);
					УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект), Ссылка);
				КонецЕсли;
			КонецЦикла;
			Возврат;	
		Иначе
			Сообщить("Для контрагента не заданы печатные формы документа.", СтатусСообщения.Важное);
		КонецЕсли;	
	КонецЕсли;
	
	Если ИмяМакета = "РН_ТТН" Тогда
		
		//1. Печать РН-УМК
		Обработка = Константы.ОбработкаПечатиРН.Получить();
		
		Если Не ЗначениеЗаполнено(Обработка)
			Тогда
			Сообщить("В константах УМК не установлена печатная форма по умолчанию");
			Возврат
		КонецЕсли;
		
		Если Не Обработка.ВидОбработки = Перечисления.ВидыДополнительныхВнешнихОбработок.ПечатнаяФорма
			Тогда
			Сообщить("В константах УМК неверно установлен вид обработки печатной формы");
			Возврат
		КонецЕсли;

		Расшифровка = Новый Структура();
		Расшифровка.Вставить("ВидПечатнойФормы", Обработка.ВидОбработки);
		Расшифровка.Вставить("ЗаменяемаяПечатнаяФорма", Неопределено);
		Расшифровка.Вставить("НомерСтроки", 1);
		Расшифровка.Вставить("СсылкаНаВнешнююОбработку",Обработка);
		ТабДокумент = УниверсальныеМеханизмы.НапечататьВнешнююФорму(Ссылка, Расшифровка);
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект), Ссылка);
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	// + umk
	СтруктураМакетов.Вставить("ПакетнаяПечать" , "ПАКЕТНАЯ ПЕЧАТЬ");
	СтруктураМакетов.Вставить("РН_ТТН" , "Печать РН+ТТН");
	// - umk
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоКомплектам, ТаблицаПоТаре, ТаблицаПоУслугам, 
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТаре Цикл
		...
		Проводка.Организация     = СтруктураШапкиДокумента.Организация;
		Проводка.Сумма           = СтрокаТаблицы.ПроводкиСуммаСНДСРегл;
		Проводка.Содержание  	 = НСтр("ru='Передача возвратной тары';uk='Передача зворотної тари'",Локализация.КодЯзыкаИнформационнойБазы());
		Проводка.НомерЖурнала    = "";
...
Процедура ПроводкиПоНДС(ПроводкиБУ, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, 
	...
	ТаблицаКопия.Свернуть("СделкаНУ, СделкаБУ, СтавкаНДС, СчетДоходовБУ, СубконтоДоходовБУ1, СубконтоДоходовБУ2, СубконтоДоходовБУ3, НоменклатурнаяГруппа, НалоговоеНазначениеДоходовИЗатрат","ПроводкиСуммаНДСРегл,ПроводкиСуммаНДСВал,ПроводкиСуммаНДСКурсНБУ,ПроводкиСуммаБезНДСВал");
	...
	ТаблицаДвижений.Свернуть("СделкаНУ, СделкаБУ, СтавкаНДС, СчетДоходовБУ, СубконтоДоходовБУ1, СубконтоДоходовБУ2, СубконтоДоходовБУ3, НоменклатурнаяГруппа, НалоговоеНазначениеДоходовИЗатрат","ПроводкиСуммаНДСРегл,ПроводкиСуммаНДСВал,ПроводкиСуммаНДСКурсНБУ,ПроводкиСуммаБезНДСВал");
...
#КонецЕсли
		...
		ЗаполнитьПоДокументуПеремещениеТоваров(Основание);
		
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПланОтгрузок") Тогда
		
		ЗаполнитьПоДокументуПланОтгрузок(Основание);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	СтруктураЗначений.Вставить("ТекущийТипЦен"						, ОбъектКопирования.ТипЦен);
	СтруктураЗначений.Вставить("ТекущийВалютаДокумента"				, ОбъектКопирования.ВалютаДокумента);
	СтруктураЗначений.Вставить("ТекущийВалютаВзаиморасчетов"		, ОбъектКопирования.ВалютаДокумента);
	СтруктураЗначений.Вставить("ТекущийКурсДокумента"				, ОбъектКопирования.КурсВзаиморасчетов);
	СтруктураЗначений.Вставить("ТекущийКурсВзаиморасчетов"			, ОбъектКопирования.КурсВзаиморасчетов);
	СтруктураЗначений.Вставить("ТекущийУчитыватьНДС"				, ОбъектКопирования.УчитыватьНДС);
	СтруктураЗначений.Вставить("ТекущийСуммаВключаетНДС"			, ОбъектКопирования.СуммаВключаетНДС);

	СтруктураЗначений.Вставить("ПерезаполнитьЦеныПоТипу"			, Истина);
	СтруктураЗначений.Вставить("НовыйТипЦен"						, ОбъектКопирования.ТипЦен);
	СтруктураЗначений.Вставить("ПересчитатьЦеныПоВалюте"			, Ложь);
	СтруктураЗначений.Вставить("НовыйВалютаДокумента"				, ОбъектКопирования.ВалютаДокумента);
	СтруктураЗначений.Вставить("НовыйКурсДокумента"					, ОбъектКопирования.КурсВзаиморасчетов);
	СтруктураЗначений.Вставить("НовыйКурсВзаиморасчетов"			, ОбъектКопирования.КурсВзаиморасчетов);
	СтруктураЗначений.Вставить("НовыйУчитыватьНДС"					, ОбъектКопирования.УчитыватьНДС);
	СтруктураЗначений.Вставить("НовыйСуммаВключаетНДС"				, ОбъектКопирования.СуммаВключаетНДС);

	СтруктураЗначений.Вставить("ТекущийКратностьДокумента"			, ОбъектКопирования.КратностьВзаиморасчетов);
	СтруктураЗначений.Вставить("НовыйКратностьДокумента"			, ОбъектКопирования.КратностьВзаиморасчетов);

	СтруктураЗначений.Вставить("ТекущийКратностьВзаиморасчетов"		, ОбъектКопирования.КратностьВзаиморасчетов);
	СтруктураЗначений.Вставить("НовыйКратностьВзаиморасчетов"		, ОбъектКопирования.КратностьВзаиморасчетов);

	СтруктураЗначений.Вставить("ТекущийАвторасчетНДС"				, ОбъектКопирования.АвторасчетНДС);
	СтруктураЗначений.Вставить("НовыйАвторасчетНДС"					, ОбъектКопирования.АвторасчетНДС);
	...
	СтруктураЗначений.Вставить("ТекущийУсловиеПродаж"                    , ОбъектКопирования.УсловиеПродаж);
	СтруктураЗначений.Вставить("НовыйУсловиеПродаж"                      , ОбъектКопирования.УсловиеПродаж);
	...
	Комментарий = "";
	umk_ОтложенноеПроведение = Ложь;
	umk_Импортирована = Ложь;
	umk_НомерЗаказаФ2 = "";
	umk_ДатаЗаказаФ2 = Неопределено;
	umk_ДатаДоставкиФ2 = Неопределено;
	umk_номерФ2 = ""; 
	НомерВПлане = 0;  
	ПланОтгрузок = Неопределено;
	
	//+EDIN
	 DATE_= Неопределено;
	 NUMBER_= Неопределено;
	 DELIVERYDATE = Неопределено;
	 DELIVERYTIME = Неопределено;
	 ORDERNUMBER = Неопределено;
	 ORDERDATE = Неопределено;
	 DELIVERYNOTENUMBER = Неопределено;
	 DELIVERYNOTEDATE = Неопределено; 
	 SHIPMENTS = Неопределено;
	 INFO = Неопределено;
	 CAMPAIGNNUMBER = Неопределено;
	 TRANSPORTTYPE = Неопределено;
	 TRANSPORTERTYPE = Неопределено;
	 EDIN = Ложь;
	//-EDIN
	
	ОсновнойДоговорКонтрагента = ЗаполнениеДокументов.ПолучитьДоговорПоОрганизацииИКонтрагенту(Организация, Контрагент, мСтруктураПараметровДляПолученияДоговора);
...
// + umk 
//Заполним документ на основании перемещения
Процедура ЗаполнитьПоДокументуПеремещениеТоваров(Основание)
	
	umk_ОснованиеПеремещение = Основание;
	
	Если Основание.umk_ДатаПеремещения Тогда
		Дата = Основание.Дата
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	ПеремещениеТоваровТовары.КоличествоМест КАК КоличествоМест,
		...
		|	ВЫРАЗИТЬ(ПеремещениеТоваровТовары.Количество - ЕСТЬNULL(ВложенныйЗапрос.КоличествоРТиУ, 0) КАК ЧИСЛО(15, 2)) КАК Количество,
		...
		|	ЕСТЬNULL(ВложенныйЗапрос.КоличествоРТиУ, 0) КАК КоличествоРТиУ
		|ИЗ
		|	Документ.ПеремещениеТоваров.Товары КАК ПеремещениеТоваровТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			РеализацияТоваровУслугТовары.Номенклатура КАК Номенклатура,
		|			СУММА(РеализацияТоваровУслугТовары.Количество) КАК КоличествоРТиУ
		|		ИЗ
		|			Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслугТовары
		|		ГДЕ
		|			РеализацияТоваровУслугТовары.Ссылка.umk_ОснованиеПеремещение = &Основание
		...
		|	ЕСТЬNULL(ВложенныйЗапрос.КоличествоРТиУ, 0),
		|	ПеремещениеТоваровТовары.НомерСтроки,
		|	ВЫРАЗИТЬ(ПеремещениеТоваровТовары.Количество - ЕСТЬNULL(ВложенныйЗапрос.КоличествоРТиУ, 0) КАК ЧИСЛО(15, 2))
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Количество < 0 Тогда
			Сообщить("Реализовано продукции " + Строка(Выборка.Номенклатура) + " на " + Строка(Выборка.Количество) + " больше чем перемещено." );
			Продолжить
		КонецЕсли;
...
//Заполним документ на основании плана отгрузок
Процедура ЗаполнитьПоДокументуПланОтгрузок(Основание)
	
	НомерВПлане = ПолучитьНомерВПлане(Основание);
	
	Если НомерВПлане = 0 Тогда
		#Если Клиент Тогда
			Сообщить("Все накладные отгружены!", СтатусСообщения.Важное);
		#КонецЕсли
		Возврат 
	КонецЕсли;
	
	ПланОтгрузок = Основание;
	...
	Дата  = Основание.Дата;
	ПредставительОрганизации = Организация.ИндивидуальныйПредприниматель;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПланОтгрузокТовары.Номенклатура,
	|	ПланОтгрузокТовары.ХарактеристикаНоменклатуры,
	|	ПланОтгрузокТовары.ЕдиницаИзмерения,
	|	ПланОтгрузокТовары.Коэффициент,
	|	ПланОтгрузокТовары.н" + Строка(НомерВПлане) + " КАК Количество
	|ИЗ
	|	Документ.ПланОтгрузок.Товары КАК ПланОтгрузокТовары
	|ГДЕ
	|   ПланОтгрузокТовары.н" + Строка(НомерВПлане) + " <> 0 И 
	|	ПланОтгрузокТовары.Ссылка = &Ссылка";
...
// Возвращает номер накладной не созданный на основании плана отгрузок
Функция ПолучитьНомерВПлане(Основание)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	РеализацияТоваровУслуг.НомерВПлане КАК НомерВПлане
	|ИЗ
	|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	|ГДЕ
	|	РеализацияТоваровУслуг.ПланОтгрузок = &ПланОтгрузок
	|	И РеализацияТоваровУслуг.ПометкаУдаления = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерВПлане";
	
	Запрос.УстановитьПараметр("ПланОтгрузок", Основание);
	...
	Пока Выборка.Следующий() Цикл
		СписокНомеров.Добавить(Выборка.НомерВПлане);
	КонецЦикла;
	
	 НомерПлана = 1;
	 
	// Обойдем все колонки номеров накладных
	Пока  НомерПлана <= 30 Цикл
		Если Не КоличествоВКОлонке(НомерПлана, Основание) Тогда
			НомерПлана =  НомерПлана + 1;
			Продолжить
		КонецЕсли;
		
		Если СписокНомеров.НайтиПоЗначению(НомерПлана) = Неопределено
			Тогда
			Возврат НомерПлана
		КонецЕсли;
		НомерПлана =  НомерПлана + 1;    	
...
// Проверим. Количество в колонке плана отгрузок не должно быть равно 0
Функция КоличествоВКОлонке(НомерПлана, Основание)
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Сумма(umk_ПланОтгрузокТовары.н" + Строка(НомерПлана) + ") КАК Количество 
	|ИЗ
	|	Документ.ПланОтгрузок.Товары КАК umk_ПланОтгрузокТовары
	|ГДЕ
	|	umk_ПланОтгрузокТовары.Ссылка = &Основание");
...
//Ответственое лицо представитель организации
Процедура УстановитьПредставителяОрганизацииКладовщика(Период) Экспорт
	
	ПредставительОрганизации = Справочники.ФизическиеЛица.ПустаяСсылка();
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ГрафикиРаботыСкладскихРаботников.Сотрудник.Физлицо КАК Сотрудник
		|ИЗ
		|	РегистрСведений.umk_ГрафикиРаботыСкладскихРаботников КАК ГрафикиРаботыСкладскихРаботников
		|ГДЕ
		|	ГрафикиРаботыСкладскихРаботников.Период = &Дата");
	...
	Если Выборка.Следующий() Тогда
	     ПредставительОрганизации = Выборка.Сотрудник;
	КонецЕсли;
...
Процедура ПриУстановкеНовогоНомера(СтандартнаяОбработка, Префикс)
	...
	Префикс = Формат(Дата,"ДФ=dd-MM-yy");
	...
	//Запрос.УстановитьПараметр("Организация", Организация);//Константы.umk_ГоловнаяОрганизация.Получить());
	//
	//РезультатЗапроса = Запрос.Выполнить();
	//
	//Выборка = РезультатЗапроса.Выбрать();
	...
	//ПоследнийНомер = Лев(ПоследнийНомер,4);
	//ПоследнийНомер = Число(ПоследнийНомер);
	//
	//Если Количество <= ПоследнийНомер Тогда
	//	Количество =  ПоследнийНомер + 1 ;
	//КонецЕсли;
	
	
	////++++
	Организация_ = Организация;
	
	Если Не ЗначениеЗаполнено(Организация_) Тогда
		Организация_ = Константы.umk_ГоловнаяОрганизация.Получить();
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		НомерДн = Формат(Число(Лев(Выборка.Номер,4))+1,"ЧЦ=4; ЧВН=; ЧГ=");
	Иначе
		НомерДн = "0001";
	КонецЕсли;
	///+++++
	...
	//НомерДн = Формат(Количество,  "ЧЦ=4; ЧВН=; ЧГ=");
	Номер = НомерДн + ?(Организация = Константы.umk_ГоловнаяОрганизация.Получить(), "",Организация.Префикс) + "-" + Префикс;
	
	Если Не ЗначениеЗаполнено(DELIVERYNOTENUMBER) Тогда  
		DELIVERYNOTENUMBER = Номер;
	КонецЕсли;
...
Процедура СоздатьУведомлениеEDIN() Экспорт
	
	Если EDIN Тогда
		Возврат
	КонецЕсли;
	
	Если  Контрагент.EDIN_расписание = 0 И Не ЗначениеЗаполнено(Контрагент.ГраницаОтправкиEDIN)
		Тогда
		Возврат
	КонецЕсли;
	
	
	Если  Контрагент.EDIN_расписание = 1  
		И Не ЗначениеЗаполнено(Контрагент["ГраницаОтправкиEDIN"+Строка(ДеньНедели(Дата))]) 		
		Тогда
		Возврат
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Ответственный)
		Тогда
		Сообщить("Ответственный в документе не заполнен, напоминание EDI не создано!");
		Возврат
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	ЗадачиПользователяЗадачиПоИсполнителю.Выполнена
		|ИЗ
		|	Задача.ЗадачиПользователя.ЗадачиПоИсполнителю(
		|			&Исполнитель,
		|			EDIN = ИСТИНА
		|				И Дата = &Дата
		|				И Объект = &Контрагент) КАК ЗадачиПользователяЗадачиПоИсполнителю";
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.Выполнена
			Тогда Возврат
		КонецЕсли;
		
		Задача = Выборка.Ссылка.ПолучитьОбъект();
		ТЧ_Документы = Задача.Документы;
		Парам = Новый Структура("Документ", Ссылка);
		ДокументыПоиска = ТЧ_Документы.НайтиСтроки(Парам);
		Если Не ДокументыПоиска.Количество() = 0
			Тогда //Нашли наш документ, ничего не делаем 
			Возврат
		Иначе
			НоваяСтрока = ТЧ_Документы.Добавить();
			НоваяСтрока.Документ = Ссылка;
			Задача.Записать();	
		КонецЕсли;
		
	Иначе // нет задачи, создаем.
		
		Задача = Задачи.ЗадачиПользователя.СоздатьЗадачу();
		Задача.Наименование = "Отправка документов в EDI по "+Строка(Контрагент.Наименование);
		Задача.Объект = Контрагент;
		Задача.Оповещение = Истина;
		Задача.EDIN = Истина;
		Задача.Дата = НачалоДня(Дата);
		//Задача.ДатаИсполнения 
		Задача.Инициатор = глЗначениеПеременной("глТекущийПользователь");
		Задача.Исполнитель = Ответственный;
		Задача.ГруппаПользователей = Справочники.ГруппыПользователей.ВыпискаДокументов_umk;
		Если Контрагент.EDIN_расписание = 0 Тогда
			Задача.СрокИсполнения = НачалоДня(Дата) + (Контрагент.ГраницаОтправкиEDIN)*60*60;
		Иначе
			Задача.СрокИсполнения = НачалоДня(Дата) + (Контрагент["ГраницаОтправкиEDIN"+Строка(ДеньНедели(Дата))])*60*60;
		КонецЕсли;
		Задача.СрокОповещения = Задача.СрокИсполнения - ?(ЗначениеЗаполнено(Контрагент.УведомитьОбОтправкеEDIN),
		Контрагент.УведомитьОбОтправкеEDIN*60*60, 3*60*60); 
		//Строка
		ТЧ_Документы = Задача.Документы;
		НоваяСтрока = ТЧ_Документы.Добавить();
		НоваяСтрока.Документ = Ссылка;
