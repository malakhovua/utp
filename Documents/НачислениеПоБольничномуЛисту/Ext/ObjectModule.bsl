...
Функция АвтозаполнениеРасчетСреднего() Экспорт
	...
	Иначе
		...
		|					ИЛИ Начисления.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.Начисления.МобилизацияБезОплаты)
		|			ТОГДА 1
		|		КОНЕЦ )	КАК ЕстьДопустимыеНевыходы
		|	ИЗ РегистрРасчета.Начисления КАК Начисления
		|	ГДЕ		Начисления.Сотрудник = &парамСотрудник
...
Функция РассчитатьСреднююПорядок1266( КомментироватьРасчет = Ложь ) Экспорт
	...
	Иначе
		...
		Если Истина Тогда
			...
			|			И	Начисления.ВидРасчета <> ЗНАЧЕНИЕ(ПланВидовРасчета.Начисления.МобилизацияБезОплаты)
			|
			|		ГДЕ		Средняя.Авторасчет		
			|			И	Средняя.Регистратор = &Регистратор			
