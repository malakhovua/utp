...
Функция ПечатьВозвратаОтПокупателя()
	...
	Для каждого ВыборкаСтрокТовары из ЗапросТовары Цикл 
		...
		ОбластьМакета.Параметры.Товар       = СокрЛП(ВыборкаСтрокТовары.Товар) + ФормированиеПечатныхФорм.ПредставлениеСерий(ВыборкаСтрокТовары,  ВыборкаСтрокТовары.ДокументПартии.Дата);// + umk ФормированиеПечатныхФорм.ПредставлениеСерий(ВыборкаСтрокТовары);
...
Функция СформироватьТаблицуВозвращенныхПартийБух(СчетУчета, Номенклатура, НоменклатурнаяПозиция, НалоговоеНазначение, ДокументОтгрузки, ВозвратКоличество, Отказ, Заголовок)
	...
	Если КоличествоНеПодобраноПартии > 0 Тогда
		...
		СообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Бух. учет. Документом %1 было реализовано %2 на %3 ед. меньше, чем возвращается.", СокрЛП(ДокументОтгрузки), Номенклатура, КоличествоНеПодобраноПартии);
		ОбщегоНазначения.ОшибкаПриПроведении(СообщенияОбОшибке, Отказ, Заголовок);
		
		// + umk  -- Временное решение, т. к. все партии остались в базе 1с 7.7
		Если ДокументОтгрузки.Дата < '20181231' тогда
			
			Отказ = Ложь;
			НоваяСтрока = ТаблицаВозвращенныхПартий.Добавить();
			НоваяСтрока.Партия = ДокументОтгрузки; // ?? не партиобразующий
			НоваяСтрока.Количество   = ВозвратКоличество;
			НоваяСтрока.СуммаСписания = umk_Доработки.СтоимостьПартииРН_77(ДокументОтгрузки, Номенклатура) * ВозвратКоличество;
			НоваяСтрока.СуммаСписанияНУ = НоваяСтрока.СуммаСписания; 
...
Процедура ПроводкиПоНДС(ПроводкиБУ, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоВторомуСобытиюНал, МассивРасчетныхДокументов, Отказ)
	...
	// ТОВАРЫ
	ТаблицаКопия = ТаблицаПоТоварам.Скопировать();                                                                                                                           			
	ТаблицаКопия.Свернуть("СтавкаНДС, СчетДоходовБУ,СубконтоДоходовБУ1,СубконтоДоходовБУ2,СубконтоДоходовБУ3,НоменклатурнаяГруппа, НалоговоеНазначениеДоходовИЗатрат","ПроводкиСуммаНДСРегл,ПроводкиСуммаНДСВал,ПроводкиСуммаНДСКурсНБУ,ПроводкиСуммаБезНДСВал");
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	umk_номерФ2 = "";
	umk_НомерДокПокупателя  = "";
	umk_Импортирована = Ложь;
	umk_ЕстьОригинал = Ложь;
